// components/product/ProductCard.jsx (add remaining time display)
...
const ProductCard = ({ product }) => {
  ...
  // Check if product is still new (within 48 hours)
  const isStillNew = product.isNew && product.newRemainingHours > 0;
      ...
      <Link to={`/product/${product.slug}`} className="block">
        ...
        <div className="relative overflow-hidden aspect-square bg-bg-secondary">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
            ...
            {/* ✅ Updated: Only show NEW badge if still within 48 hours */}
            {isStillNew && (
              <span
                className="inline-flex items-center gap-1 px-3 py-1 bg-success/90 backdrop-blur-sm 
                         text-white text-[10px] sm:text-xs font-bold rounded-full 
                         shadow-lg"
              >
